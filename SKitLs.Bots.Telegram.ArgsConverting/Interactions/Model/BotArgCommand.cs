...
{
    public class BotArgCommand<TArg> : DefaultCommand, IArgedAction<TArg, SignedMessageTextUpdate> where TArg : new()
    {
        public char SplitToken { get; set; }

        public BotArgCommand(string @base, BotInteraction<SignedMessageTextUpdate> action, char splitToken = ';')
            : base(@base, action)
        {
