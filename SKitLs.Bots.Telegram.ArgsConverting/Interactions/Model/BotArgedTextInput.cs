...
{
    /// <summary>
    /// Represents a text input action with argument support, extending the functionality of <see cref="DefaultTextInput"/>.
    ...
    /// <b>Example:</b>
    /// <para/>
    /// For the input data <c>'Random 100'</c> with the default split token <c>' '</c> and an <see cref="int"/> argument,
    /// '100' would be parsed as an argument.
    /// </summary>
    ...
    public class BotArgedTextInput<TArg> : DefaultTextInput, IArgedAction<TArg, SignedMessageTextUpdate> where TArg : notnull, new()
    {
        /// <summary>
        /// <inheritdoc/>
        /// <para/>
        /// The default split token is <c>' '</c>. For the data <c>'Random 100'</c>, '100' would be parsed as an argument.
        /// </summary>
        ...
        public BotArgedTextInput(string @base, BotArgedInteraction<TArg, SignedMessageTextUpdate> action, char splitToken = ' ') : base(@base, null!)
        {
            Action = MiddleAction;
            ArgAction = action;
