...
{
    public abstract class InputProcess<TResult> : DefaultProcessBase, IBotProcess<InputProcessArgs<TResult>>
    {
        public virtual InputProcessComplete<TResult> WhenOver { get; protected set; }

        public InputProcess(string processDefId, string terminationalKey, IUserState controllerState, InputProcessComplete<TResult> whenOver) : base(processDefId, terminationalKey, controllerState)
        {
            WhenOver = whenOver;
        }

        public virtual async Task Terminate(InputProcessResult<TResult> result, SignedMessageTextUpdate update)
        {
        ...
        public abstract IBotRunningProcess GetRunning(long userId, InputProcessArgs<TResult> args);
    }
