...
namespace SKitLs.Bots.Telegram.DataBases.Extensions
{
    public static class FillHelper
    {
        ...
        public static List<PartialSubProcess<T>> GetSubInputs<T>(T @object, string terminationalKey, int detalizationId = 0) where T : IBotPartialInput
        {
            ...
            foreach (var prop in props)
            {
                ...
                if (attr is PartialInputAttribute partial)
                {
                    ...
                    var textBuilderDelegate = textBuilderMethod is null
                        ...
                        : (Func<ISignedUpdate, IOutputMessage>)Delegate.CreateDelegate(typeof(Func<ISignedUpdate, IOutputMessage>), @object, textBuilderMethod);

                    var sub = new PartialSubProcess<T>(prop, new DynamicMessage(textBuilderDelegate!), parserDelegate is not null ? new ParseInputDelegate(parserDelegate) : null);
                ...
                else if (attr is SelectorInputAttribute selector)
                {
                    ...
                    var sub = new PartialSubProcess<T>(prop, new DynamicMessage(u => mes), ListParse)
                    {
